...
(function () {
  ...
  if (classList.includes('tabsch_website')) {
    ...
    const webResultEls = document.querySelectorAll(
      `div.sp_website.section ul.type01>li`
    );
  ...
  if (classList.includes('tabsch_video')) {
    const currentKeyword = document.querySelector('#nx_query').value;
    const videoResultEls = document.querySelectorAll(
      `div.sp_video.section ul>li`
    );
  ...
  if (classList.includes('tabsch_blog')) {
    const currentKeyword = document.querySelector('#nx_query').value;
    const blogResultEls = document.querySelectorAll(
      `div.blog.section ul.type01>li`
    );
  ...
  if (classList.includes('tabsch_kin')) {
    const currentKeyword = document.querySelector('#nx_query').value;
    const kinResultEls = document.querySelectorAll(
      `div.kinn.section ul.type01>li`
    );
  ...
  if (classList.includes('tabsch_cafe')) {
    const currentKeyword = document.querySelector('#nx_query').value;
    const searchResultEls = document.querySelectorAll(
      '#elThumbnailResultArea li'
    );
    searchResultEls.forEach((searchResultEl) => {
  ...
  if (!classList.length) {
    ...
    // 검색 결과에 이벤트 리스너 달기
    // 뉴스
    const newsResultEls = document.querySelectorAll(
      `div.news.section ul.type01>li`
    );
    ...
    // 쇼핑
    const shoppingResultEls = document.querySelectorAll(
      `div.sp_shop_default.section .group_guide ul>li`
    );
    ...
    // 카페
    const cafeResultEls = document.querySelectorAll(
      `div.cafe.section ul.type01>li`
    );
    ...
    // 포스트
    const postResultEls = document.querySelectorAll(
      `div.sp_post.section ul.type01>li`
    );
